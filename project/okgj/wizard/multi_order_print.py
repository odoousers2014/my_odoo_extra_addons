...
class okgj_stock_picking_collect_temp(osv.osv):
    ...
    _columns = {
        'collect_ids':fields.many2many('okgj.multi.order.print', 'picking_multi_rel', 'picking_id', 'multi_id', u'拣货单'),
        'okgj_print_seq':fields.text(u'位置号'),
        'inv_print_state':fields.boolean(u'已开发票'),
    }
...
class okgj_multi_order_print(osv.osv):
    ...
    _columns = {
        ...
        'picking_ids':fields.many2many('stock.picking', 'picking_multi_rel', 'multi_id', 'picking_id', u'发货单', domain=[('type', '=', 'out')], required=True),
        'line_ids':fields.one2many('okgj.multi.order.print.line', 'print_id', 'Lines'),
        'seq':fields.text(u'位置关系'),
        'inv_print_state':fields.boolean(u'已开发票'),
    }
    
    _defaults = {
        'name': lambda obj, cr, uid, context: obj.pool.get('ir.sequence').get(cr, uid, 'okgj.multi.order.print'),
    }
    ...
    def _prepare_multi_line(self, cr, uid, picking_ids, warehouse_id, context=None):
        ...
        for one_picking in self.pool.get('stock.picking.out').browse(cr, uid, picking_ids, context=context):
            ...
            #写入打印位置号
            picking_obj.write(cr, uid, one_picking.id, {'okgj_print_seq':seq}, context=context)
            seq_str += picking_name + ' : ' + str(seq) + '\n'
            ...
            for one_move in move_lines:
                ...
                if print_product.get(product_id, False):
                    ...
                    if one_move.prodlot_id and one_move.prodlot_id.name:
                        print_product[product_id]['distribute'] +=  '\n' + one_move.prodlot_id.name + ' || '+ str(product_real_qty) + ' || ' +  str(seq) + ' || ' + picking_name
                    else:
                        print_product[product_id]['distribute'] +=  '\n' + str(product_real_qty) + ' || ' + str(seq) + ' || ' + picking_name
                else:
                    product_rack = one_move.pick_rack_id.name or one_move.store_rack_id.name or None
                    #分布处理
                    if one_move.prodlot_id and one_move.prodlot_id.name:
                        prod_distribute =  one_move.prodlot_id.name + ' || '+ str(product_real_qty) + ' || ' +  str(seq) + ' || ' + picking_name
                    else:
                        prod_distribute =  str(product_real_qty) +' || ' +  str(seq) + ' || ' + picking_name
        ...
        res.update({'line_ids':values, 'seq':seq_str}) 
        return res
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        vals.update({
            'name':self.pool.get('ir.sequence').get(cr, uid, 'okgj.multi.order.print'),
            'picking_ids':[(6, 0, picking_ids)],
            'warehouse_id':warehouse_id})
        self.pool.get('stock.picking.out').write(cr, uid, picking_ids, {
            'print_state':'yes',
            'inv_print_state':vals['inv_print_state'],
            }, context=context)
