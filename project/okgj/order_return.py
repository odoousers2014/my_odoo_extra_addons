...
class okgj_purchase_return(osv.osv):
    ...
    _description = 'OKGJ Purchase Return'
...
class okgj_purchase_return_line(osv.osv):
    ...
    def _get_qty_available(self, cr, uid, ids, field_names, arg, context=None):
        if context is None:
            context = {}
        result = {}.fromkeys(ids, 0)
        ...
        for one_line in self.browse(cr, uid, ids, context=context):
            if one_line.product_id:
                product_qty_available = product_obj.browse(cr, uid, one_line.product_id.id,context={'warehouse':warehouse_id}).qty_available
                result[one_line.id] = product_qty_available
        return result
    
    _name = "okgj.purchase.return.line"
    _description = 'OKGJ Purchase Return Line'
    _columns = {
        ...
        'partner_id':fields.related('return_order_id', 'partner_id', type='many2one', relation='res.partner', string=u'供应商', readonly=True, store=True),
        'qty_available':fields.function(_get_qty_available, type='float', string=u'当前库存数量', store=True),
    }
...
class okgj_purchase_return(osv.osv):
    ...
    def _test_repeat(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for one_order in self.browse(cr, uid, ids, context):
            product_ids = []
            line_count = len(one_order.line_ids)
            product_count = len(set([one_line.product_id  for one_line in one_order.line_ids]))
            if product_count < line_count:
                result[one_order.id] = True
            else:
                result[one_order.id] = False
        return result
    ...
    _columns = {
        ...
        'has_repeat': fields.function(_test_repeat, type='boolean', string=u'有重复行', store=True),
    }
    _defaults = {
    ...
    def action_show_repeat(self, cr, uid, ids, context=None):
        if isinstance(ids, list):
            ids = ids[0]
        product_ids = {}
        for one_line in self.browse(cr, uid, ids, context=context).line_ids:
            one_product = one_line.product_id
            if one_product.id in product_ids:
                raise osv.except_osv(_(u'Warn!'), _(u'第%s行与%s行商品重复!') % (product_ids[one_product.id],one_line.line_no))
            else:
                product_ids.update({one_product.id:one_line.line_no})
        return True
    ...
    def action_cancel(self, cr, uid, ids, context=None):
        if context is None:
            context = {}
        picking_obj = self.pool.get('stock.picking.out')
        return_picking_ids = picking_obj.search(cr, uid, [
                ('okgj_type', '=', 'okgj_purchase_out'), 
                ('purchase_return_id', 'in', ids)], context=context)
        for picking in picking_obj.browse(cr, uid, return_picking_ids, context=context):
            if picking.state not in ('cancel'):
                raise osv.except_osv((u'无效动作!'), (u'请先取消该采购退货申请单的相关出库单!'))
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        for picking in picking_obj.browse(cr, uid, picking_ids, context=context):
            raise osv.except_osv(_('错误!'), _('要删除采购退货申请单 %s.,请先删除其出库单!') %(picking.purchase_return_id.name))
        return super(okgj_purchase_return, self).unlink(cr, uid, ids, context=context)
...
class okgj_sale_return_old_line(osv.osv):
    ...
    _description = "Okgj.Sale.Return.Old.Line"
    _columns = {
        'product_id':fields.many2one('product.product', u'商品', required=True),
        ...
        'sale_order_id':fields.related('return_order_id', 'sale_order_id', type='many2one', relation='sale.order', string=u'源订单号', store=True),
        'price_unit':fields.float(u'单价', digits_compute=dp.get_precision('Product Price')),
        ...
        'default_code': fields.related('product_id', 'default_code', type='char', size=128, string="Product default code", store=True, select=True),
    }
...
class okgj_sale_return_new_line(osv.osv):
    ...
    _description = "Okgj.Sale.Return.New.Line"
    _columns = {
        'product_id':fields.many2one('product.product', u'商品', required=True),
        'variant':fields.related('product_id', 'variants', type="char", string=u"规格", readonly=True),
        'product_qty':fields.float(u'数量', digits_compute=dp.get_precision('Product Unit of Measure')),
...
class okgj_sale_return(osv.osv):
    ...
    _columns = {
        ...
        'okgj_city':fields.related('sale_order_id', 'okgj_city', type='char', string=u'收货城市', readonly=True, store=True),
        'region_name':fields.text(u'收货区域'),
        'address':fields.text(u'收货地址', states={'draft':[('readonly', False)]}, readonly=True),
        'origin_sale_pay_id': fields.related('sale_order_id', 'pay_name', type="char", string=u'源付款方式', readonly=True, store=True),
