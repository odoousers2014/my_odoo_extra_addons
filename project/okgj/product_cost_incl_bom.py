...
#===============================================================================
    ...
    # }
    #===========================================================================
    def _group_product_weight_check(self, cr, uid, ids, context=None):
        for p in self.browse(cr, uid, ids, context=context):
            if p.weight - p.weight_bom > 0.01:
                return True
        return True
    
    def _weight_bom(self, cr, uid, ids,field_name,arg=None, context=None):
        return self._compute_weight(cr, uid, ids, context=context)
    
    def _compute_weight(self, cr, uid, ids, context=None):
        if context is None:
            context = {}
        product_uom = context.get('product_uom')
        bom_properties = context.get('properties', [])
        ...
        if not ids:
            return computed
        _logger.debug("_compute_weight with ids %s" % ids)
        ...
        sub_read = self.read(cr, uid,
                             list(chain.from_iterable(depends.itervalues())),
                             ['weight', 'uom_po_id'], context=context)    
        subproduct_costs = dict((p['id'], p) for p in sub_read)
        ...
        no_bom_ids = [p_id for p_id in ordered if
                      p_id not in product_bom and
                      p_id in ids]
        costs = super(product_product, self)._compute_weight(
            cr, uid, no_bom_ids, context=context)
        computed.update(costs)
        ...
        for product_id in ordered:
            ...
            for subproduct_info in subproduct_infos:
                ...
                # the subproducts are always computed before their
                # parents
                subcost = computed.get(subproduct_id, subproduct['weight'])
                qty = uom_obj._compute_qty(
                    cr, uid,
                    from_uom_id=subproduct_info['product_uom'],
                    qty=subproduct_info['product_qty'],
            ...
            cost = uom_obj._compute_price(
                ...
                cost, bom.product_id.uom_id.id)
            computed[product_id] = cost
        ####syn weight by weight_bom, don.t use write method, avoid circulation write  
        for pid in computed:
            if pid not in no_bom_ids:
                product=self.browse(cr, uid, pid, context=context)
                template_id = product.product_tmpl_id.id
                cr.execute('update product_template set(weight)=(%s) where id=%s', (computed[pid],template_id) )
        ####syn weight by weight_bom,
        return computed

    _weight_bom_triggers={
        'product.product': (_get_bom_product, None, 5),
        'mrp.bom': (_get_product,
                    ['bom_id',
                     'bom_lines',
    ...
    _columns = {
        ...
        #=======================================================================
        'weight_bom':fields.function(
            _weight_bom,
            store=_weight_bom_triggers,
            string='Weight (incl. BoM)',
            type='float',
            digits=(8,2),
            help="Weight by bom",
                    ),
    ...
    _constraints=[
        #(_group_product_weight_check, u'组合品重量价低于bom累加重量，请重新设置', [ 'weight_bom', 'weight', 'is_group_product']),
    ]
