...
class excel_sale_order(osv.osv_memory):
    ...
    def check_sale_order(self, cr, uid, model, title_data):
        ...
        bad_line = ''
        no_sku_line = ''
        if model == 'beibeiwang' or model == 'yangmatou':
        ...
        for line in title_data['data']:
            ...
            if model == 'beibeiwang':
                ...
                for data in datas:
                    ...
                    sale_order_line_id = sale_order_line_obj.create(cr, uid, order_line_vals, context=context)
            # if freight > 0:
            #     order_line_vals.update({
            #     'delay': 0,
            #     'name': "[%s] %s" % (product_datas['PostFee2']['default_code'], product_datas['PostFee2']['name']),
            #     'order_id': sale_id,
            #     'price_unit': 1,
            #     'product_uom': 1,
            #     'product_uom_qty': int(freight),
            #     'state': 'draft',
            #     'type': 'make_to_stock',
            #     'product_id': product_datas['PostFee2']['id'],
            # })
            # sale_order_line_obj.create(cr, uid, order_line_vals, context=context)
        return res_ids
